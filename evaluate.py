...
def run_evaluation(output_dir, gold_dir, eval_json, result_dir, timeout_seconds):
    ...
    print("====================================")
    print(df.groupby("type").agg({"score": "mean", "finished": "mean", "type": "count"}).rename(columns={"type": "count"}))
    print("-------------------------------")
    print(df.groupby("hardness").agg({"score": "mean", "finished": "mean", "hardness": "count"}).rename(columns={"hardness": "count"}))
    print("-------------------------------")
    print(df.groupby("big_type").agg({"score": "mean", "finished": "mean", "big_type": "count"}).rename(columns={"big_type": "count"}))
    print("-------------------------------")
    print(df.groupby("result_type").agg({"score": "mean", "finished": "mean", "result_type": "count"}).rename(columns={"result_type": "count"}))

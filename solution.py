...
file_handler = logging.FileHandler(os.path.join("da_agent/logs", "normal-{:}.log".format(datetime_str)), encoding="utf-8")
debug_handler = logging.FileHandler(os.path.join("da_agent/logs", "debug-{:}.log".format(datetime_str)), encoding="utf-8")
stdout_handler = logging.StreamHandler(sys.stdout)
sdebug_handler = logging.FileHandler(os.path.join("da_agent/logs", "sdebug-{:}.log".format(datetime_str)), encoding="utf-8")
...
def config() -> argparse.Namespace:
    ...
    parser.add_argument("--suffix", '-s', type=str, default="100-round4")

    # parser.add_argument("--model", '-m', type=str, default="Qwen/Qwen2.5-72B-Instruct")
    parser.add_argument("--model", '-m', type=str, default="Pro/deepseek-ai/DeepSeek-V3")
    ...
    # example config
    parser.add_argument("--task_config", "-t", type=str, default="dataset/DACode/configs/task/example.jsonl")
    parser.add_argument("--source_dir", type=str, default="dataset/DACode/source")
    ...
    # output related
    parser.add_argument("--output_dir", type=str, default="/home/gaohao/hdd/data/dsagent/output")
...
def start(args: argparse.Namespace) -> None:
    ...
    # load task configs
    assert os.path.exists(args.task_config) and args.task_config.endswith(
        ".jsonl"), f"Invalid task_config, must be a valid jsonl file: {args.task_config}"
    ...
    for task_config in task_configs:
        ...
        if args.retry_failed and os.path.exists(result_json_path):
            with open(result_json_path, "r") as f:
                result = json.load(f)
                if result["finished"] and (not "FAIL" in result["result"]) and (
                        not "error" in result["result"].lower()):
                    logger.info("Skipping %s", instance_id)
                    continue
        ...
        result_files = env.post_process(trajectory['trajectory'])
        dabench_result = {"finished": done, "steps": len(trajectory["trajectory"]), "total_tokens": agent.get_total_tokens(),
